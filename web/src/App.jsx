...
export default function App() {
  ...
  // 顶部导航视图切换：chat=百图通对话台，settings=模型配置（运维）。
  // 隐藏而非卸载百图通视图——保留地图实例、会话列表与进行中的对话状态
  const [view, setView] = useState("chat");

  return (
    <div className="app-shell">
      <header className="topnav">
        <span className="topnav-brand">百图通 <i>AI-GIS</i></span>
        <nav className="topnav-tabs">
          <button type="button" className={view === "chat" ? "active" : ""}
            onClick={() => setView("chat")}>
            💬 百图通
          </button>
          <button type="button" className={view === "settings" ? "active" : ""}
            onClick={() => setView("settings")}>
            ⚙ 模型配置
          </button>
        </nav>
      </header>
      <div className={`console${showMap ? " has-map" : ""}${view === "settings" ? " hidden" : ""}`}>
        <SessionSidebar
          sessions={sessions}

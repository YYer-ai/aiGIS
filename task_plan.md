...
## 2026-08-27 UX 优化轮（自主模式）

- feature/ux-streaming 已合并 main（114 passed）：SSE 流式（SQL 逐字+阶段+计时）、结果卡片（大数字/表格前10行）、地图要素 popup、Enter/chips/自动滚动、start_web.ps1 一键启动
- 真实浏览器验收全过（含 popup 内容、流式中间态、139 行表格截断）
- 尾修：表格隐藏 geometry 列、启动脚本端口检查/构建中止、Decimal 显示净化
